...
paymentrouter.post("/payment", async (req: Request, res: Response) => {
    ...
    try {
        ...
        const sslcz = new SSLCommerzPayment("hr6799e6efb7586", "hr6799e6efb7586@ssl", false); // Use environment variables!
...
paymentrouter.post("/payment/success/:tranId", async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (result.modifiedCount > 0) {
            await cart.deleteMany({ _id: { $in: order.items.map((item: any) => item._id) } });
...
paymentrouter.post("/payment/fail/:tranId", async (req: Request, res: Response) => {
    try {
        const result = await Order.deleteOne({ transaction: req.params.tranId });

        if (result.deletedCount > 0) {
            return res.redirect(`http://localhost:5173/payment/fail/${req.params.tranId}`); // Include tran_id as query parameter
        } else {
